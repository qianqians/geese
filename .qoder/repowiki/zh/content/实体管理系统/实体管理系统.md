...
**新增内容**：本次更新重点补充了物理组件（PhysicsComponent）的集成，包括物理系统与实体管理的协同工作方式、物理组件的使用模式以及在实体生命周期中的应用。
...
- 实体与玩家：entity、player
- 子实体：subentity
- 物理系统：physics、physics_component
- 生命周期与持久化：save
- 服务与路由：service
- 应用入口与注册：app
- 接收器：receiver
...
subgraph "物理系统"
PHYS["physics<br/>物理世界与场景封装"]
PHCOMP["physics_component<br/>物理组件"]
end
subgraph "运行时"
...
- [server/engine/physics.py:1-275](file://server/engine/physics.py#L1-L275)
- [server/engine/physics_component.py:1-95](file://server/engine/physics_component.py#L1-L95)
...
- 物理系统：physics 提供物理世界的薄封装，支持场景管理、刚体创建和物理计算；physics_component 作为轻量组件绑定实体与物理刚体。
...
- [server/engine/physics.py:1-275](file://server/engine/physics.py#L1-L275)
- [server/engine/physics_component.py:17-95](file://server/engine/physics_component.py#L17-L95)
...
### 物理组件集成与使用模式
- 物理组件设计
  - PhysicsComponent 采用组件风格，不侵入 base_entity，业务实体在 __init__ 中显式 self.physics = PhysicsComponent(...)。
  - 支持多种刚体类型：dynamic（动态）、fixed/static（固定）、kinematic/kinematic_position（运动学位置）、kinematic_velocity（运动学速度）。
- 物理系统架构
  - physics 提供薄封装：World（物理世界）、Scene（场景）、Body（刚体）、Shape（形状）。
  - physics_component 作为轻量组件绑定实体与物理刚体，提供位置、旋转、速度等属性访问和物理操作。
- 生命周期集成
  - 实体销毁时需调用 physics_component.destroy() 移除物理刚体，避免悬挂引用。
  - 物理组件与实体的 full_info/hub_info/client_info 无缝集成，支持跨服同步。
...
class World {
+create_scene(gravity) Scene
+destroy_scene(scene) bool
+contains_scene(id) bool
}
class Scene {
+step(dt)
+add_dynamic(shape, pos, rot, ...)
+add_fixed(shape, pos, rot, ...)
+add_kinematic(shape, pos, rot, ...)
+cast_ray(origin, direction, max_toi)
}
class Body {
+position() Vec3
+rotation() Quat
...
PhysicsComponent --> Scene : "绑定场景"
PhysicsComponent --> Body : "持有刚体"
World --> Scene : "管理场景"
```

**图表来源**
- [server/engine/physics_component.py:17-95](file://server/engine/physics_component.py#L17-L95)
- [server/engine/physics.py:25-275](file://server/engine/physics.py#L25-L275)
...
- 组件耦合
  ...
  - physics_component 依赖 physics 模块，提供物理功能的轻量封装。
...
SAVE --> CTX
CTX --> HUB["HubContext"]
PHCOMP["physics_component"] --> PHYS["physics"]
PHCOMP --> ENT
PHCOMP --> PLR
```
...
- [server/engine/physics_component.py:87-92](file://server/engine/physics_component.py#L87-L92)
...
- 物理组件问题
  - 物理组件 destroy 返回 False 通常表示刚体已被移除或不存在。
  - 物理组件 is_alive 可用于检查刚体状态，但销毁后应避免继续使用。
...
- [server/engine/callback.py:17-23](file://server/engine/callback.py#L17-L23)
- [server/engine/physics_component.py:84-92](file://server/engine/physics_component.py#L84-L92)

## 结论
geese 实体管理系统以清晰的分层与统一的上下文封装，实现了实体生命周期的完整闭环、动态类型扩展、跨服迁移与一致性保障、子实体的缓存与恢复、完善的事件与回调体系以及高效的异步与持久化策略。本次更新补充了物理组件的集成，通过 PhysicsComponent 与 physics 模块的协作，为实体提供了丰富的物理交互能力。该方案适合高并发、多服协同的游戏场景，具备良好的可扩展性与稳定性。
...
- 关键流程速查
  ...
  - 物理组件：PhysicsComponent 初始化 → 场景添加刚体 → 实体销毁时 destroy