...
| 4.5 | 阴影 CSM | P1 | M2 | 🟢 骨架完成（[crates/render/src/shadow.rs](file://crates/render/src/shadow.rs)，6 单测） |
| 4.6 | IBL/天空盒 | P1 | M2 | 🟢 骨架完成（[crates/render/src/ibl.rs](file://crates/render/src/ibl.rs)，5 单测） |
| 4.7 | 后处理（Bloom/TAA/Tone Mapping） | P1 | M2 | 🟢 骨架完成（[crates/render/src/post.rs](file://crates/render/src/post.rs)，6 单测） |
| 4.8 | 粒子系统 | P1 | M2 | 🟢 骨架完成（[crates/vfx](file://crates/vfx/)，7 单测） |
| 4.9 | GPU Skinning | P1 | M2 | 🟢 骨架完成（[crates/render/src/skinning.rs](file://crates/render/src/skinning.rs)，7 单测） |
| 4.10 | AOI 兴趣管理 | P1 | M3 | 🟢 骨架完成（[crates/aoi](file://crates/aoi/)，7 单测） |
| 4.11 | 状态同步与插值 | P1 | M3 | 🟢 骨架完成（[crates/sync](file://crates/sync/)，9 单测） |
| 4.12 | NavMesh 寻路 | P2 | M3 | 🟢 骨架完成（[crates/navmesh](file://crates/navmesh/)，7 单测） |
| 4.13 | 地形与流式加载 | P2 | M3 | 🟢 骨架完成（[crates/terrain](file://crates/terrain/)，8 单测） |
...
- **骨架 API**：`CascadeConfig`、`CsmUniform`/`CascadeUniform`（std140 对齐）、`compute_cascade_splits()`（PSSM 混合 lambda）、`compute_atlas_layout()`（1/2/4 格布局）、`DirectionalShadowCaster`、`ShadowAtlas` trait + `NullShadowAtlas`、`MAX_CASCADES = 4`。
- **里程碑**：🟢 M2 骨架完成（6 单测，零新依赖）→ wgpu shadow pass + cluster shading 阴影采样 → PCF/PCSS。

### 4.6 IBL/天空盒 [crates/render](file://crates/render)（扩展）
- **目标**：HDR 天空盒、Irradiance Cubemap、Prefiltered Cubemap、BRDF LUT。
- **骨架 API**：`IblConfig`、`SkyboxKind`（Hdri/Procedural/SolidColor）、`IblResources`、`IblUniform`、`IblBaker` trait + `StubIblBaker`。
- **依赖**：4.4 资源管线（HDR/KTX2 加载）。
- **里程碑**：🟢 M2 骨架完成（5 单测，零新依赖）→ 静态 cubemap + Hammersley/GGX 烘焙 → 运行时动态 IBL。
...
- **骨架 API**：`PostEffect` enum、`PostChain`、`PostUniform`、`EffectMask`（手写位掋避免 bitflags 依赖）、`aces_tonemap()`、`halton_2_3()`（TAA jitter）、`build_post_uniform()`。
- **影响面**：新增 post-process pass chain、抖动相机矩阵接入 [crates/camera](file://crates/camera/)。
- **里程碑**：🟢 M2 骨架完成（6 单测：ACES 边界 + Halton 序列 + chain 顺序 + uniform 掋码，零新依赖）→ ACES pass → Bloom → TAA。

### 4.8 粒子系统 [crates/vfx](file://crates/vfx)（新建）
- **目标**：CPU 粒子发射器、GPU 粒子（compute shader）、Billboard/Mesh 粒子。
- **骨架 API**：`Particle`、`Emitter`、`EmitterShape`（Point/Sphere/Cone/Box）、`BillboardKind`（ViewAligned/AxisLocked/VelocityAligned/Mesh）、`ParticleSystem::tick(dt)`、手写 `Rng`（MINSTD LCG 避免 rand 依赖）、`lerp_color()`。
- **依赖**：4.4 资源管线（纹理/材质）。
- **里程碑**：🟢 M2 骨架完成（7 单测：RNG 确定性 + 生命周期 + max_particles 反压力，零新依赖）→ GPU 粒子（wgpu compute）→ Trail/Mesh 粒子。

### 4.9 GPU Skinning [crates/render](file://crates/render) ↔ [crates/avatar](file://crates/avatar)（扩展）
- **现状**：CPU 端 `compute_joint_matrices` + uniform 上传；M2 骨架已在 [crates/render/src/skinning.rs](file://crates/render/src/skinning.rs) 面向多模式抽象。
- **骨架 API**：`SkinningMode`（Cpu/UniformPalette/JointSsbo/VertexPulling/Morph）、`GpuJointMatrix`、`JointPalette`、`MorphWeights`、`compute_joint_matrices()`、`SkinningUploader` trait + `NullSkinningUploader`、`MAX_MORPH_TARGETS=8`。
...
- **里程碑**：🟢 M2 骨架完成（7 单测：mode 能力位 + palette 截断 + identity 乘法 + morph 夾碌，零新依赖）→ joint SSBO 接入 → vertex pulling → morph targets。

### 4.10 AOI 兴趣管理 [crates/aoi](file://crates/aoi)（新建，服务端）
- **目标**：九宫格 / Octree AOI、Enter/Leave/Update 事件。
- **骨架 API**：`Aoi` trait、`GridAoi`（九宫格 + insert/update/remove + 双向邻居重算）、`AoiEvent::Enter/Leave`、`observers(target)`、`take_events()`。
- **里程碑**：🟢 M3 骨架完成（7 单测：enter/leave/remove/observers/re-insert，零新依赖）→ 与 [server/engine/entity.py](file://server/engine/entity.py) 集成 → 动态 Octree AOI。

### 4.11 状态同步与插值 [crates/sync](file://crates/sync)（新建） + [server/engine](file://server/engine/) + [client/engine](file://client/engine/)（后续接入）
- **目标**：滞后插值（Source Engine 模式）、预测/回滚（Overwatch 模式可选）。
- **骨架 API**：`Snapshot`（server_time + pos/vel/rot）、`SnapshotBuffer`（环形缓冲 + 乱序丢包）、`InterpolationMode::Lagged/Extrapolated`、`sample(render_time, mode)`、`interpolate()`（四元数 nlerp 归一化）、`predict_position()`、`recommended_interp_delay(tick_hz)`。
- **依赖**：4.10 AOI（确定同步范围）。
- **里程碑**：🟢 M3 骨架完成（9 单测：环形容量 + 中点插值 + 超出范围 + 外推速度 + nlerp 归一化，零新依赖）→ client/server 接入 → 预测/回滚。

### 4.12 NavMesh 寻路 [crates/navmesh](file://crates/navmesh)（新建）
- **目标**：基于 NavMesh 的 A*/funnel 寻路、动态 obstacle。
- **骨架 API**：`Vec2`、`NavTri`、`NavMesh::from_triangles()`（自动建邻接）、`NavMesh::locate(p)`（point-in-tri）、`find_path(start, end)`（三角形邻接图上的 A*）、`funnel_smooth()`（路径平滑，现以共享边中点连接）、`area_cost` 区域代价。
- **选型**：后续接入 `oxidized_navigation 0.10`（Recast & Detour Rust 端口）跟 Recast 体素生成对齐。
- **依赖**：4.10 AOI（场景分块）。
- **里程碑**：🟢 M3 骨架完成（7 单测：邻接构建 + locate + A* + 不可达返 None + funnel 端点，零新依赖）→ 接入 oxidized_navigation 烘焙 → 运行时动态 obstacle。

### 4.13 地形与流式加载 [crates/terrain](file://crates/terrain)（新建）
- **目标**：高度图地形、LOD、流式加载（streaming）。
- **骨架 API**：`Heightmap`（bilinear 采样 + 中心差分法线）、`TerrainTile`（coord + heightmap + world_origin + lod）、`TileCoord`、`compute_lod()`（geo-clipmap 风格，距离翻倍 +1 级）、`TileLoader` trait + `NullTileLoader`、`TerrainStreamer::update(camera_xz, loader)`（按 view_radius 加/卸 tile）。
- **选型**：Geo-clipmap LOD、tile-based streaming。
- **依赖**：4.4 资源管线、4.5 CSM（大场景阴影）、4.12 NavMesh（地形导航网格）。
- **里程碑**：🟢 M3 骨架完成（8 单测：bilinear 边界 + 平面法线 + LOD 阶梯 + Streamer 加/卸，零新依赖）→ clipmap LOD 接入渲染 → 磁盘/网络 tile loader。
