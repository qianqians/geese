...
- [Cargo.toml（physics）](file://crates/physics/Cargo.toml)
...
- 物理系统：`crates/physics` 基于 rapier3d 0.32 封装多场景物理世界，仅运行于服务端；通过 pyo3 feature 挂载到 `pyhub` Python 模块，由 [physics.py](file://server/engine/physics.py)与 [physics_component.py](file://server/engine/physics_component.py) 面向业务 entity 提供组件化接入。
