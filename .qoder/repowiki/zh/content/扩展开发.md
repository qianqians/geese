...
## 更新摘要
**所做更改**
- 更新八叉树系统章节以反映重大重构：从存储完整 SceneObject 改为存储 OctreeEntry 元数据条目
- 新增静态/动态对象分类系统的设计与实现
- 改进视锥剔除查询逻辑，提升性能和正确性
- 更新场景管理章节以包含新的对象分类机制
- 更新性能优化建议以适应新的八叉树架构
...
- **八叉树系统：重大重构从存储完整 SceneObject 改为存储 OctreeEntry 元数据条目，新增静态/动态对象分类系统**
- **场景管理与相机控制的扩展开发**
...
- **动画系统：avatar（新增）**
- **八叉树系统：scene crate 内部的 Octree 实现**
- 网络与协议：net、tcp、wss
- 时间与队列：time、queue
...
- [crates/scene/src/octree.rs:1-390](file://crates/scene/src/octree.rs#L1-L390)
...
- **八叉树系统**（**重大重构**）
  - **OctreeEntry 元数据条目**：节点中只保存 `OctreeEntry { id, aabb, center }`，不持有 `SceneObject` 副本，避免每帧 clone。
  - **静态/动态对象分类**：通过 `classify_objects` 函数将对象分为静态（进八叉树）和动态（每帧线性测试）两类。
  - **改进的视锥剔除**：使用 `intersects_aabb` 而非 `contains_aabb`，确保跨界对象不被误剔除。
  - **性能优化**：静态对象使用八叉树加速，动态对象直接线性测试，避免每帧重建。
- **场景与对象**
...
**图表来源**
- [crates/scene/src/scene.rs:93-110](file://crates/scene/src/scene.rs#L93-L110)
- [crates/scene/src/octree.rs:236-241](file://crates/scene/src/octree.rs#L236-L241)
...
+max_objects : usize
+max_depth : usize
+insert(id, aabb, center)
+query_frustum(frustum) : Vec<usize>
+clear()
+len() : usize
+is_empty() : bool
}
...
**图表来源**
- [crates/scene/src/scene.rs:16-31](file://crates/scene/src/scene.rs#L16-L31)
- [crates/scene/src/scene.rs:93-110](file://crates/scene/src/scene.rs#L93-L110)
- [crates/scene/src/octree.rs:204-258](file://crates/scene/src/octree.rs#L204-L258)
- [crates/scene/src/octree.rs:21-26](file://crates/scene/src/octree.rs#L21-L26)
...
#### 八叉树系统重大重构
**重要更新**：八叉树系统已从存储完整 SceneObject 重构为存储 OctreeEntry 元数据条目，显著提升了性能和正确性。

#### OctreeEntry 元数据条目
- **设计要点**
  - **轻量级存储**：节点中只保存 `OctreeEntry { id, aabb, center }`，不持有 `SceneObject` 副本。
  - **避免克隆开销**：调用方根据返回的 id 索引到自己的对象数组，避免每帧 clone。
  - **解耦设计**：octree 与具体对象类型解耦，提高模块独立性。
- **典型用法**
  ```rust
...
#### 静态/动态对象分类系统
- **设计要点**
  - **分类函数**：`classify_objects` 根据动画 channel 的 target_node 集合 + 其后代节点 + 拥有 skin 的对象进行分类。
  - **保守判断**：宁可让对象动态化（性能略差但正确性安全）。
  - **性能优化**：静态对象进入八叉树加速，动态对象直接线性测试。
- **分类规则**
  - **动态对象**：拥有皮肤（skinned）或动画影响的节点
  - **静态对象**：无动画影响且无皮肤的独立对象

#### 改进的视锥剔除查询逻辑
- **设计要点**
  - **正确性提升**：使用 `intersects_aabb` 而非 `contains_aabb`，确保跨界对象不被误剔除。
  - **边界处理**：改进的细分逻辑，确保中心落在分割面上的对象不会丢失。
  - **性能优化**：静态对象使用八叉树查询，动态对象直接线性测试。

```mermaid
flowchart TD
Start(["开始"]) --> Classify["classify_objects 分类"]
Classify --> Static["静态对象进八叉树"]
Classify --> Dynamic["动态对象线性测试"]
Static --> Insert["Octree::insert(id, aabb, center)"]
Insert --> Query["Octree::query_frustum(frustum)"]
Query --> Map["映射ID到SceneObject"]
Dynamic --> LinearTest["逐个AABB测试"]
Map --> Merge["合并结果"]
LinearTest --> Merge
Merge --> End(["完成"])
```
...
**图表来源**
- [crates/scene/src/scene.rs:294-332](file://crates/scene/src/scene.rs#L294-L332)
- [crates/scene/src/scene.rs:93-110](file://crates/scene/src/scene.rs#L93-L110)
- [crates/scene/src/octree.rs:226-241](file://crates/scene/src/octree.rs#L226-L241)
...
- **设计要点**
  ...
  - **静态对象使用八叉树加速**：通过 Octree::query_frustum 进行空间分割查询。
  - **动态对象直接线性测试**：通过 `frustum.intersects_aabb` 进行 AABB 相交测试。
- **使用建议**
...
- **场景与动画**
  ...
  - **八叉树系统优化**：
    - **静态对象分类**：通过 classify_objects 将对象分为静态和动态两类，静态对象进入八叉树加速，动态对象直接线性测试。
    - **元数据存储**：OctreeEntry 只存储必要的元数据（id, aabb, center），避免每帧克隆完整 SceneObject。
    - **正确性提升**：使用 intersects_aabb 而非 contains_aabb，确保跨界对象不被误剔除。
    - **边界处理**：改进的细分逻辑，确保中心落在分割面上的对象不会丢失。
    - **性能优化**：静态对象使用八叉树查询，动态对象直接线性测试，避免每帧重建。
...
- **场景问题**
  - **八叉树问题**：
    - **静态对象消失**：检查 classify_objects 分类是否正确，确保静态对象被正确识别。
    - **动态对象不显示**：确认动态对象的 AABB 是否正确更新，检查 frustum.intersects_aabb 调用。
    - **越界对象丢失**：检查 Octree::insert 的 debug_assert 是否触发，确保对象中心在根边界内。
    - **跨界对象被剔除**：验证使用 intersects_aabb 而非 contains_aabb 的查询逻辑。
...
- [crates/scene/src/octree.rs:226-234](file://crates/scene/src/octree.rs#L226-L234)
- [crates/scene/src/scene.rs:93-110](file://crates/scene/src/scene.rs#L93-L110)
...
## 结论
geese 的扩展开发围绕"场景-渲染-网络"三大主线展开。**通过将动画系统重构到独立的 avatar crate、渲染系统重构为双路径架构，以及八叉树系统重大重构，开发者现在可以**：
...
- **通过八叉树系统的重大重构，实现静态/动态对象分类、元数据存储优化和视锥剔除正确性提升**；
...
- **开发建议**
  ...
  - **对于八叉树系统，建议先理解静态/动态对象分类机制，再实现相应的对象管理逻辑**。
...
- **工具与脚手架**
  ...
  - **利用八叉树系统的测试用例，验证静态/动态对象分类和视锥剔除的正确性**。