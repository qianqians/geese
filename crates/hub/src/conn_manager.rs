...
pub struct ConnManager {
    ...
    dbproxys: BTreeMap<String, Arc<StdMutex<DBProxyProxy>>>,
    hubproxys: BTreeMap<String, Arc<StdMutex<HubProxy>>>,
    gateproxys: BTreeMap<String, Arc<StdMutex<GateProxy>>>
}
...
impl ConnManager {
        ...
        -> Option<Arc<Mutex<Box<dyn NetWriter + Send + 'static>>>>
    {
        ...
        if let Ok((rd, wr)) = TcpConnect::connect(host.clone()).await {
            ...
            let _conn_proxy = Arc::new(StdMutex::new(
    ...
    pub async fn add_dbproxy_proxy(&mut self, _proxy: Arc<StdMutex<DBProxyProxy>>) {
        let _db = _proxy.as_ref().lock().unwrap();
        self.dbproxys.insert(_db.dbproxy_name.clone(), _proxy.clone());
    }

    pub fn get_dbproxy_proxy(&mut self, _dbproxy_name: &String) -> Option<&Arc<StdMutex<DBProxyProxy>>> {
        self.dbproxys.get(_dbproxy_name)
    }

    pub fn add_hub_proxy(&mut self, _name: String, _proxy: Arc<StdMutex<HubProxy>>) {
        self.hubproxys.insert(_name, _proxy.clone());
    }

    pub fn get_hub_proxy(&mut self, _hub_name: &String) -> Option<&Arc<StdMutex<HubProxy>>> {
        self.hubproxys.get(_hub_name)
    }

    pub fn add_gate_proxy(&mut self, _name: String, _proxy: Arc<StdMutex<GateProxy>>) {
        self.gateproxys.insert(_name, _proxy.clone());
    }

    pub fn get_gate_proxy(&mut self, _gate_name: &String) -> Option<&Arc<StdMutex<GateProxy>>> {
