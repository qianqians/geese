...
def gen_entity_module(module_name, funcs, dependent_struct, dependent_enum, enum):
    ...
    code_constructor += "    def __init__(self, entity:player):\n"
    ...
    for i in funcs:
        ...
        if i[1] == "ntf":
            ...
            code_func += "    def " + func_name + "(self, hub_name:str, msg_cb_id:int, bin:bytes):\n"
            code_func += "        inArray = loads(bin)\n"
            count = 0
            for _type, _name, _parameter in i[2]:
                type_ = check_type(_type, dependent_struct, dependent_enum)
                ...
                code_func += gen_type_code_module(
                    ...
                    dependent_enum)
                count += 1
            code_func += "        rsp = " + module_name + "_" + func_name + "_rsp(hub_name, msg_cb_id, self.entity)\n"
            code_func += "        for fn in self.on_" + func_name + ":\n"
            code_func += "            fn(rsp, "
            count = 0
            for _type, _name, _parameter in i[2]:
            ...
            rsp_code += "    def __init__(self, hub_name:str, msg_cb_id:int, entity:player):\n"
            rsp_code += "        session.__init__(self, hub_name)\n"
            ...
            for _type, _name, _parameter in i[4]:
                ...
                rsp_code += gen_type_code_type_to_protcol(
                    ...
                    dependent_struct, 
                    dependent_enum)
            rsp_code += "        self.entity.call_hub_response(self.msg_cb_id, dumps(_argv_" + _argv_uuid + "))\n\n"
            ...
            for _type, _name, _parameter in i[6]:
                ...
                rsp_code += gen_type_code_type_to_protcol(
                    ...
                    dependent_struct, 
                    dependent_enum)
            rsp_code += "        self.entity.call_hub_response_error(self.msg_cb_id, dumps(_argv_" + _argv_uuid + "))\n\n"
