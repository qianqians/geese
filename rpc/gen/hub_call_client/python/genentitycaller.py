...
def gen_entity_caller(module_name, funcs, dependent_struct, dependent_enum, enum):
    ...
    code += "    def __init__(self, entity:player):\n"
        ...
        if i[1] == "ntf":
            ...
            for _type, _name, _parameter in i[2]:
                ...
                code += gen_type_code_type_to_protcol(
                    ...
                    dependent_struct, 
                    dependent_enum)
            code += "        self.entity.call_client_main_notify(\"" + func_name + "\", dumps(_argv_" + _argv_uuid + "))\n\n"
            ...
            cb_code += "    def __init__(self, _cb_uuid:int, _entity:player):\n"
            cb_code += "        self.entity = _entity\n"
            cb_code += "        self.cb:" + rsp_fn + " = None\n"
            cb_code += "        self.err:" + err_fn + " = None\n"
            cb_code += "        self.rsp = callback(lambda: self.entity.del_client_callback(_cb_uuid))\n"
            cb_code += "        self.entity.reg_client_callback(_cb_uuid, self.rsp)\n\n"
            ...
            for _type, _name, _parameter in i[2]:
                ...
                code += gen_type_code_type_to_protcol(
                    ...
                    dependent_struct, 
                    dependent_enum)
            code += "        _cb_uuid = self.entity.call_client_request(\"" + func_name + "\", dumps(_argv_" + _argv_uuid + "))\n\n"
